...
export function TransactionRow({
  ...
  return (
    <div className="group relative flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-gray-50 transition-colors">
      {/* Ícone da categoria */}
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          <p className="text-sm font-medium text-gray-900 truncate">{transacao.descricao_raw}</p>
      ...
      <div className="flex-shrink-0 text-right">
        <p className={`text-sm font-semibold tabular-nums ${isReceita ? "text-green-600" : "text-gray-900"}`}>
          {isReceita ? "+" : "-"}R$ {Math.abs(transacao.valor).toFixed(2)}
        </p>
      </div>

      {/* Ações rápidas — desktop: aparece no hover | mobile: sempre visível */}
      <div className="flex-shrink-0 relative">
        <button
          onClick={() => setMenuAberto(!menuAberto)}
          aria-label="Ações"
          className="md:opacity-0 md:group-hover:opacity-100 opacity-60 transition-opacity p-1.5 rounded-lg hover:bg-gray-100 text-gray-400 hover:text-gray-600"
        >
        ...
        {menuAberto && (
          <>
            ...
            <div className="absolute right-0 top-9 z-20 bg-white rounded-xl border border-gray-100 shadow-lg py-1 w-48">
              <button
                onClick={() => {
                  onEdit(transacao);
                  setMenuAberto(false);
