...
const whatsappWebhookRoutes: FastifyPluginAsync = async (fastify) => {
  fastify.post('/whatsapp/webhook', async (request, reply) => {
    ...
    try {
      ...
      const numeroRemetente = remoteJid.replace('@s.whatsapp.net', '').replace(/\D/g, '')
      ...
      if (error) {
        ...
        await enviarMensagemWhatsApp(numeroRemetente, '❌ Erro ao registrar. Tente novamente em alguns instantes.')
        return
      }
      ...
      await enviarMensagemWhatsApp(numeroRemetente, mensagemResposta)
